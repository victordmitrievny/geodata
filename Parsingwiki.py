...
def find_information(country):
	...
	#return results
	return { "Capital": find_capital(tbody),
...
#---------------------------Lesha's code for finding country names + finding population
# for ind, t_row in enumerate(row_list):
#     if ind > 2:
#         country_name = t_row.a.text
#         country_list.append(country_name)

# def find_population_v2(country):
# 	url = "https://en.wikipedia.org/wiki/" + country
# 	response = requests.get(url)
# 	soup = BeautifulSoup(response.content, 'lxml')
# 	infobo = soup.find('table', class_="infobox ib-country vcard")
# 	#Exception for Netherlands+
# 	if infobo == None:
# 		infobo = soup.find('table', class_="infobox ib-pol-div vcard")
# 	#Exception for Western Sahara+
# 	if infobo == None:
# 		infobo = soup.find('table', class_="infobox ib-settlement vcard")
# 	tbody = infobo.find('tbody')
# 	for tr in tbody:
...
# 				try:
# 					population = content.strip().replace(',', '')
# 					if population.isdigit():
# 						return population 
# 				except Exception:
# 				    pass
# print(find_population_v2('United_States'))
# print(' __')
